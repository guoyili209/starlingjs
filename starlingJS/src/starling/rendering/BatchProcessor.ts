...
module StarlingJS {
    export class BatchProcessor {
        private _batches: Array<MeshBatch>;
        private _batchPool: BatchPool;
        private _currentBatch: MeshBatch;
        private _currentStyleType: any;
        private _onBatchComplete: Function;
        private _cacheToken: BatchToken;

        // helper objects
        private static sMeshSubset: MeshSubset = new MeshSubset();

        /** Creates a new batch processor. */
        constructor() {
            this._batches = new Array<MeshBatch>();
        ...
        dispose(): void {
            for (var batch: MeshBatch of this._batches)
                batch.dispose();
         ...
         */
        addMesh(mesh: Mesh, state: RenderState, subset?: MeshSubset,
        ...
        getBatchAt(batchID: int): MeshBatch {
        ...
        fillToken(token: BatchToken): BatchToken {
        ...
        get numBatches(): int { return _batches.length; }
        ...
        get onBatchComplete(): Function { return _onBatchComplete; }
        set onBatchComplete(value: Function): void { _onBatchComplete = value; }
    }
}
...
class BatchPool {
    private _batchLists: Dictionary;
